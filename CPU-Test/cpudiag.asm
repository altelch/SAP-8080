...
;(805) 527-0518 (VERBAL)
;
;Modified to run without CP/M H.K. 2023
;
;
;
;
...
TTYO: EQU   011H  ; Device ID 1 for TTY Out
;
;
;
;MESSAGE OUTPUT ROUTINE
;
MSG:  PUSH PSW     ; Save Flags, A
MSGC: MOV  A,M     ; Move value at MEM(H,L) -> A
      ORA  A       ; OR A with itself -> Z true if A==0
      JZ   MSGF    ; Msg finished if A==0
      INX  H       ; Next character
      OUT  TTYO    ; Output character
      JMP  MSGC    ; Continue output
MSGF: POP  PSW     ; Restore Flags, A
      RET          ; Return
;
;
;
...
;
PCHAR: OUT  TTYO    ; Output character
	     RET
;
;
;
...
;************************************************************
;
PROG:   DB	0CH,0DH,0AH,' MICROCOSM ASSOCIATES 8080/8085 CPU DIAGNOSTIC',00H
;
VERS:  	DB	0AH,' VERSION 1.0  (C) 1980',00H
;
OKCPU:	DB	0AH,' CPU IS OPERATIONAL',00H
;
NGCPU:	DB	0AH,' CPU HAS FAILED!    ERROR EXIT=',00H
;
;
;
;************************************************************
;                8080 CPU TEST/DIAGNOSTIC
;************************************************************
;
;NOTE: (1) PROGRAM ASSUMES "CALL",AND "LXI SP" INSTRUCTIONS WORK!
;
;      (2) INSTRUCTIONS NOT TESTED ARE "HLT","DI","EI" AND "RST 0"
;
;
;
JUMP:   DB  0AH,' TEST JUMP INSTRUCTIONS AND FLAGS',00H
;
...
;
;
ACCU:   DB  0AH,' TEST ACCUMULATOR IMMEDIATE INSTRUCTIONS',00H
;
AIMM: LXI	H,ACCU
	CALL	MSG
  CPI	0	;A=F5H,C=0,Z=0
...
;
;
CALRET:   DB  0AH,' TEST CALLS AND RETURNS',00H
;
C010:	LXI	H,CALRET
	CALL	MSG
  ANI	000H	;A=0,C=0,P=1,S=0,Z=1
	CC	CPUER	;TEST "CC"
	CPO	CPUER	;TEST "CPO"
	CM	CPUER	;TEST "CM"
	CNZ	CPUER	;TEST "CNZ"
...
;
;
MOVR:   DB  0AH,' TEST "MOV","INR",AND "DCR" INSTRUCTIONS',00H
;
MOVI:	LXI	H,MOVR
	CALL	MSG
  MVI	A,077H
...
;
;
ARLOG:   DB  0AH,' TEST ARITHMETIC AND LOGIC INSTRUCTIONS',00H
;
	LXI	H,ARLOG
	CALL	MSG
	XRA	A
	...
	CMP	L
	CNZ	CPUER	;TEST "PUSH H" AND "POP H"
	LXI	H,00000H  ; HL = 00000H
	DAD	SP        ; HL = HL + SP -> HL = SP
	SHLD	SAVSTK	; SAVE THE "OLD" STACK-POINTER!
	LXI	SP,TEMP4  ; SP = 0814H (TEMP4)
	DCX	SP        ; SP = 0813H (TEMP3)
	DCX	SP        ; SP = 0812H (TEMP2)
	INX	SP        ; SP = 0813H (TEMP3)
	DCX	SP        ; SP = 0812H (TEMP2)
	MVI	A,055H
	STA	TEMP2     ; TEMP2=055H
	CMA           ; A=0AAH
	STA	TEMP3     ; TEMP3=0AAH
	POP	B         ; B=0AAH
	CMP	B
	...
	CMP	C
	CNZ	CPUER	;TEST "LXI","DAD","INX", AND "DCX" SP
	LXI	H,TEMP4 ; OLD Stack-Pointer -> H/L
	SPHL ; SP = HL
	LXI	H,07733H
	DCX	SP
	DCX	SP
...
;
;
RSTO:   DB  0AH,' TEST RST INSTRUCTIONS ',00H
;
	LXI	H,RSTO
	CALL	MSG
	; Test RST
...
;
;
TEMP0:	DS	1	;TEMPORARY STORAGE FOR CPU TEST MEMORY LOCATIONS 0810H
TEMP1:	DS	1	;TEMPORARY STORAGE FOR CPU TEST MEMORY LOCATIONS 0811H
TEMP2:  DS	1	;TEMPORARY STORAGE FOR CPU TEST MEMORY LOCATIONS 0812H
TEMP3:	DS	1	;TEMPORARY STORAGE FOR CPU TEST MEMORY LOCATIONS 0813H
TEMP4:	DS	1	;TEMPORARY STORAGE FOR CPU TEST MEMORY LOCATIONS 0814H
;
;
;
SAVSTK:	DS	2	;TEMPORARY STACK-POINTER STORAGE LOCATION        0820H-0821H
;
;
;
STACK	EQU	1000H	;DE-BUG STACK POINTER STORAGE AREA
;
;
;
	END
