...
; Memory location equates
  org  ROM              ;ROM location

ROM      equ  0f800h    ;ROM location
SPTR     equ  ROM-0100h ;stack pointer (use 256 byte boundary)
SIOPORT  equ  SPTR-32   ;2SIO port used for hex load
BOOT     equ  0ff00h    ;Disk boot loader ROM
...
monit:
  ...
  db  CR,LF,LF,'ALTMON 1.1',080h

; start - command processing loop

start:
  lxi  sp,SPTR                    ;re-init stack pointer
...
cmdTbl:
  dw  doboot  ;B jump to Altair disk boot loader
...
cycl:
  mvi  a,'.'          ;display '.' before each cycle
  call  ptcn
  call  rndm
...
doMove:
  mov  b,a       ;save move/exchange flag in b
  call  tahex    ;read addresses
  push  h
  call  ahex
  xchg
...
nexch:
  mov  m,c       ;move source to destination
...
doSrch:
  push  psw        ;save 1/2 byte flag on stack
  call  tahex
...
flush:
  in  COND         ;clear possible received char
  lxi  d,10417     ;.25s timeout for 48 cycle loop

flshLp:
  in  CONS         ;(10) look for character on console
...
inWait0:
  call  cntlc    ;test for character from console
  jz  inWait0
  jmp  haveChr
...
inWait1:
  call  cntlc    ;look for ctrl-c on console
...
;------------------------------------------------------------
tahex:
  call  ahex     ;get first address param
...
;------------------------------------------------------------
dspMsg:
  pop  h         ;hl->string to display

dspLoop:
  mov  a,m       ;a=next character to display
...
;------------------------------------------------------------
bmp:
  mov  a,e       ;compare lsb's of hl,de
  sub  l
  jnz  goon      ;not equal
...
; an hard disk, and executes the successfully loaded code.
; Progress and error messages are printed on a "standard"
...
; PROM address and Entry point for HDBL.

HDBL	equ	0FF00h	;Beginning of HDBL PROM

; RAM address for moved code. Exactly one of these
; should be used.

RAMPAG	equ	00100h	;beginning of RAM page
...
SPT     equ	256	  ;sectors/track
RETRYS	equ	16		;max retries per sector

; Bank switching device
BANKDEV equ 18H ;Bank switching device. 0=ROM mapped, 1=RAM-Only
...
COPLOOP:
  dcx	h		              ;(5+1)
	...
	mov	m,a		            ;(7+1)
	mov	a,l		            ;(4+1)
	sui	RAMCODE and 0FFh	;(7+2)ends with a=0
	jnz	COPLOOP	          ;(10+3)
...
;----------------------------------------
  ...
  mvi  b,0       ;Track  = 0
  mvi  c,0       ;Sector = 0
  call RLBA-ROF  ;Read Block 0
  lxi  h,BOOTDST ;Load data destination
  call MOVE-ROF  ;Move Sector data to dest.
  ...
  mvi  b,0       ;Track  = 0
	mvi  c,1       ;Sector = 1
  call RLBA-ROF  ;Read Block 1
  call MOVE-ROF  ;Move Sector data to dest.

  jmp  BOOTDST   ;Run code
...
LOOP:
  ...
  jnz  LOOP-ROF  ;Repeat if not finished
  ret            ;return

;===Subroutine=================================================
;Set LBA and read sector
;Track/Sector in Reg. b/c
;==============================================================
RLBA:	
  mov	a,c        ;Set sector
  out	SECTOR

  mov a,b        ;Set Track LSB
  out	TRACK

  xra a          ;Set Track MSB (0)
  out TRACK2     ;Track MSB -> 0
  out DRIVE      ;Drive 0 (a still 0)

  mvi a,IDEREAD  ;Set Read Command, clears Counter
  out IDECOM     ;Send IDE Command
...
HDWAIT:	
  ...
  jc  HDWAIT-ROF    ;Still busy
  rlc               ;Rotate Drive Ready bit to carry
  jnc HDWAIT-ROF    ;no: wait for drive ready
  rrc               
  rrc               
...
FOREVR:
  jmp	FOREVR-ROF	  ;N: die here, INTE light lit
